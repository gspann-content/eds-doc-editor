...
export default function decorate(block) {
  ...
  const title = getMetadata('og:title');
    ...
    for (let i = 0; i < length; i += 1) {
      url = `${url}/${pathname[i]}`;
      const linkText = (i === length - 1) ? title : pathname[i].charAt(0).toUpperCase() + pathname[i].slice(1);
      ...
      if (i < length) {
        ...
        const breadcrumbLi = li({ class: 'tw-flex tw-items-center tw-ml-4' },
          divSvg,
          breadcrumbLink
        );
